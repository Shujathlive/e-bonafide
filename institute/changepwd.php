...
$sql=sprintf("SELECT institutePassword FROM institute WHERE instituteUsername='$uname'");
$res=mysqli_query($conn,$sql);
$row=mysqli_fetch_row($res);
$currpwd=$row[0];

if($oldpwd == $currpwd){
	$sql1=sprintf("UPDATE `institute` SET `institutePassword`='$newpwd' WHERE  `instituteUsername`='$uname'");
